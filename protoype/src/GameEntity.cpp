...
Vector2 GameEntity::Pos(SPACE space) {
  ...
  Vector2 scale = Vector2(mPos.x * parentScale.x, mPos.y * parentScale.y);
  Vector2 rotPos = RotateVector(scale, mParent->Rotation(local));
...
void GameEntity::Parent(GameEntity *parent) {
  ...
  if(!parent) {
    ...
    Vector2 rel = Pos(world) - parent->Pos(world);
    mPos = RotateVector(rel, -parent->Rotation(world));
